...
post <- extract.samples(m6.5)
mu.link <- function(m) post$Intercept + post$mass*m + post$`I(mass^2)`*m^2 + post$`I(mass^3)`*m^3 + post$`I(mass^4)`*m^4 + post$`I(mass^5)`*m^5
mass.seq <- seq(from=30, to=70, length.out=100)
mu <- sapply(mass.seq, mu.link)
mu.PI <- apply(mu, 2 , PI)
mu.mean <- apply(mu, 2, mean)

plot(brain~mass, data=d, col=rangi2, ylim=c(0,2000))
lines(mass.seq, mu.mean)
shade(mu.PI, mass.seq)
```
...
# multivariate neocortex + log mass model
m6.14 <- map(
  alist(
    kcal.per.g ~ dnorm(mu, exp(log.sigma)),
...
```{r, fig.align='center', fig.width=3, fig.height=5}
...
```{r, echo=FALSE}
# compute counterfactual plot of neocortex predicting calories
neo.seq <- seq(from=0.5, to=0.8, length.out=30)
d.predict <- data.frame(
  kcal.per.g = rep(0,30), # empty outcome
  neocortex = neo.seq,
  mass = rep(4.5,30) # average of mass
)

post <- extract.samples(m6.14)
mu.link <- function(n, m) post$a + post$bn * n + post$bm * log(m)
mu <- mapply(mu.link, d.predict$neocortex, d.predict$mass)
mu.mean <- apply(mu, 2, mean)
mu.PI <- apply(mu, 2, PI)

plot(d$kcal.per.g~d$neocortex, col=rangi2)
lines(neo.seq, mu.mean, lty=2)
lines(neo.seq, mu.PI[1,], lty=2)
lines(neo.seq, mu.PI[2,], lty=2)

# plot counterfactual of ensemble model
milk.ensemble <- ensemble(m6.11, m6.12, m6.13, m6.14, data=d.predict)
mu.mean <- apply(milk.ensemble$link, 2, mean)
